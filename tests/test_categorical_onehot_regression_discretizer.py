"""Fidelity tests: ``CategoricalOneHotDiscretizer`` vs ``sklearn.tree.DecisionTreeRegressor``.
...
def test_categorical_onehot_regression_criterion_aliases_equivalent(
    ...
    for crit in (alias, canonical):
        disc = CategoricalOneHotDiscretizer(criterion=crit)
        disc.Train(x, features, y, None, 1, 0.0, 0, 0)
